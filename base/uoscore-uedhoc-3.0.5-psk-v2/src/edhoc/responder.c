		  ...
		  struct byte_array *c_i)
{
	...
	/*for EDHOC-PSK*/
		...
		TRY(ciphertext_gen_psk(CIPHERTEXT2_PSK, &rc->suite, &c->c_r, &NULL_ARRAY, &NULL_ARRAY, &NULL_ARRAY, &NULL_ARRAY, &c->ead_2, &PRK_2e, &th2,
				   &ciphertext_2, &plaintext_2));
		      ...
		      struct byte_array *initiator_pk)
{
	...
	if (!rc->is_psk){
		...
		TRY(ciphertext_decrypt_split_psk(CIPHERTEXT3A_PSK, &rc->suite, NULL, &id_cred_psk, NULL, NULL, &ctxt3b,
			NULL, &rc->prk_3e2m,
		...
		TRY(ciphertext_decrypt_split_psk(CIPHERTEXT3B_PSK, &rc->suite, NULL, &id_cred_psk, &cred_i, &cred_r, NULL,
			&rc->ead, &rc->prk_4e3m,
