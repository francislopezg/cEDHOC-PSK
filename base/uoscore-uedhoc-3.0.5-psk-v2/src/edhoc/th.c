...
{
	int32_t th4_input_len;
	if (c_x_is_encoded_int(id_cred)) {
		th4_input_len =
	...
	th4_input_len =
			AS_BSTR_SIZE(get_hash_len(alg)) + AS_BSTR_SIZE(id_cred->len) + cred_i->len + cred_r->len;

	BYTE_ARRAY_NEW(id_cred_psk_enc, AS_BSTR_SIZE(ID_CRED_PSK_SIZE),
				   AS_BSTR_SIZE(id_cred->len));
	TRY(encode_bstr(id_cred, &id_cred_psk_enc));
	PRINT_ARRAY("id_cred_enc", id_cred_psk_enc.ptr, id_cred_psk_enc.len);

	BYTE_ARRAY_NEW(th4_input, TH4_PSK_INPUT_SIZE, th4_input_len);

	TRY(th4_psk_input_encode(th3, &id_cred_psk_enc, plaintext_3b, cred_i, cred_r, &th4_input));
	TRY(hash(alg, &th4_input, th4));
	PRINT_ARRAY("TH4", th4->ptr, th4->len);
	return ok;
