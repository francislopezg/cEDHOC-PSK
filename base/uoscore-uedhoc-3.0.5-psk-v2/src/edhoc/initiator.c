			     ...
			     struct byte_array *PRK_3e2m)
{
	...
	if (!is_psk){
		...
		TRY(ciphertext_decrypt_split_psk(CIPHERTEXT2_PSK, &rc->suite, c_r, NULL, NULL, NULL, NULL,
					     &rc->ead, &PRK_2e, &th2,
					     &ciphertext, &plaintext));
			      ...
			      struct byte_array *prk_out)
{
	...
	TRY(ciphertext_gen_psk(CIPHERTEXT3B_PSK, &rc->suite, &NULL_ARRAY, &c->id_cred_psk, &c->cred_i, &cred_r_array->ptr->cred, &NULL_ARRAY,
				&c->ead_3, &rc->prk_4e3m, th3,
			   &ciphertext_b, &plaintext_b));

	/*create plaintext3a and ciphertext3a*/
	TRY(ciphertext_gen_psk(CIPHERTEXT3A_PSK, &rc->suite, &NULL_ARRAY, &c->id_cred_psk, &NULL_ARRAY, &NULL_ARRAY, &ciphertext_b,
